...
!function (a) {
    ...
    function f(a, b, c) {
        ...
        var f = e(a.r, b.r, c), g = e(a.b, b.b, c), h = e(a.g, b.g, c);
        return "#" + f + h + g
    }
    ...
    function g(e, h) {
        ...
        function m(a) {
            ...
            return O = window.parseFloat(h.starWidth.replace("px", "")), U.find("svg").attr({
                width: h.starWidth,
                height: b
            }), V.find("svg").attr({width: h.starWidth, height: b}), k(), e
        }

        function o(a) {
            return h.spacing = a, Q = parseFloat(h.spacing.replace("px", "")), U.find("svg:not(:first-child)").css({"margin-left": a}), V.find("svg:not(:first-child)").css({"margin-left": a}), k(), e
        }

        function p(a) {
            return h.normalFill = a, U.find("svg").attr({fill: h.normalFill}), e
        }

        function q(a) {
            if (h.multiColor) {
                var b = X - W, c = b / h.maxValue * 100, d = h.multiColor || {}, g = d.startColor || n.startColor, i = d.endColor || n.endColor;
                a = f(g, i, c)
            } else Z = a;
            return h.ratedFill = a, V.find("svg").attr({fill: h.ratedFill}), e
        }
        ...
        function s(b) {
            ...
            for (var c = 0; c < h.numStars; c++)U.append(a(l)), V.append(a(l));
        ...
        function x(a) {
            var b = a % N, c = N / 2, d = h.halfStar, e = h.fullStar;
            return e || d ? (e || d && b > c ? a += N - b : (a -= b, b > 0 && (a += c)), a) : a
        }

        function y(a) {
            var b = U.offset(), c = b.left, d = c + U.width(), e = h.maxValue, f = a.pageX, g = 0;
            if (c > f)g = W; else if (f > d)g = e; else {
                var i = (f - c) / (d - c);
                if (Q > 0) {
                    i *= 100;
                    for (var j = i; j > 0;)j > P ? (g += N, j -= P + R) : (g += j / P * N, j = 0)
                } else g = i * h.maxValue;
                g = x(g)
            }
            return g
        ...
        function A(a) {
            var d = a, f = h.maxValue;
            return "string" == typeof d && ("%" === d[d.length - 1] && (d = d.substr(0, d.length - 1), f = 100, t(f)), d = parseFloat(d)), c(d, W, f), d = parseFloat(d.toFixed(h.precision)), b(parseFloat(d), W, f), h.rating = d, j(), Y && e.trigger("rateyo.set", {rating: d}), e
        }
        ...
        function E(a) {
            var c = y(a).toFixed(h.precision), d = h.maxValue;
            c = b(parseFloat(c), W, d), j(c), e.trigger("rateyo.change", {rating: c})
        }

        function F() {
            j(), e.trigger("rateyo.change", {rating: h.rating})
        }
        ...
        var N, O, P, Q, R, S, T = a("<div/>").addClass("jq-ry-group-wrapper").appendTo(e), U = a("<div/>").addClass("jq-ry-normal-group").addClass("jq-ry-group").appendTo(T), V = a("<div/>").addClass("jq-ry-rated-group").addClass("jq-ry-group").appendTo(T), W = 0, X = h.rating, Y = !1, Z = h.ratedFill;
        this.rating = function (a) {
            ...
            var b = Array.prototype.slice.apply(arguments, []), c = b.slice(1), e = this[a];
            ...
            return d(b) ? c(b) : h[a]
        }, s(h.numStars), z(h.readOnly), this.collection.push(this), this.rating(h.rating, !0), Y = !0, e.trigger("rateyo.init", {rating: h.rating})
    }
    ...
    var l = '<?xml version="1.0" encoding="utf-8"?><svg version="1.1"xmlns="http://www.w3.org/2000/svg"viewBox="0 12.705 512 486.59"x="0px" y="0px"xml:space="preserve"><polygon points="256.814,12.705 317.205,198.566 512.631,198.566 354.529,313.435 414.918,499.295 256.814,384.427 98.713,499.295 159.102,313.435 1,198.566 196.426,198.566 "/></svg>', m = {
        starWidth: "32px",
        normalFill: "gray",
        ratedFill: "#f39c12",
        numStars: 5,
        ...
        startColor: "#c0392b",
        endColor: "#f1c40f"
    }, o = /^#([0-9a-f]{2})([0-9a-f]{2})([0-9a-f]{2})$/i, p = function (a) {
        if (!o.test(a))return null;
        var b = o.exec(a), c = parseInt(b[1], 16), d = parseInt(b[2], 16), e = parseInt(b[3], 16);
        return {r: c, g: d, b: e}
    };
